...
def build_other_settings_tab(self):
    ...
    dav_title = ctk.CTkLabel(dav_frame, text="webdav设置", font=get_font("large"))
    ...
    create_label_with_help(self, parent=dav_warp_frame, label_text="Webdav URL", tooltip_key="webdav_url",row=0, column=0, font=get_font("button"), sticky="w")
    dav_url_entry = ctk.CTkEntry(dav_warp_frame, textvariable=self.webdav_url_var, font=get_font("button"))
    dav_url_entry.grid(row=0, column=1, padx=5, pady=5, sticky="w")

    create_label_with_help(self, parent=dav_warp_frame, label_text="Webdav用户名", tooltip_key="webdav_username",row=1, column=0, font=get_font("button"), sticky="w")
    dav_username_entry = ctk.CTkEntry(dav_warp_frame, textvariable=self.webdav_username_var, font=get_font("button"))
    dav_username_entry.grid(row=1, column=1, padx=5, pady=5, sticky="w")

    create_label_with_help(self, parent=dav_warp_frame, label_text="Webdav密码", tooltip_key="webdav_password",row=2, column=0, font=get_font("button"), sticky="w")
    dav_password_entry = ctk.CTkEntry(dav_warp_frame, textvariable=self.webdav_password_var, font=get_font("button"), show="*")
    ...
    # 测试连接按钮
    test_btn = ctk.CTkButton(button_frame, text="测试连接", font=get_font("button"),
                            command=test_webdav_connection)
    test_btn.pack(side="left", padx=5)
    
    # 保存设置按钮
    save_btn = ctk.CTkButton(button_frame, text="备份", font=get_font("button"),
                            command=backup_to_webdav)
    save_btn.pack(side="left", padx=5)
    
    # 重置按钮
    reset_btn = ctk.CTkButton(button_frame, text="恢复", font=get_font("button"),
