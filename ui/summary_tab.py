...
def build_summary_tab(self):
    ...
    load_btn = ctk.CTkButton(self.summary_tab, text="加载 global_summary.txt", command=self.load_global_summary, font=get_font("button"))
    load_btn.grid(row=0, column=0, padx=5, pady=5, sticky="w")

    self.word_count_label = ctk.CTkLabel(self.summary_tab, text="字数：0", font=get_font("button"))
    self.word_count_label.grid(row=0, column=1, padx=5, pady=5, sticky="w")

    save_btn = ctk.CTkButton(self.summary_tab, text="保存修改", command=self.save_global_summary, font=get_font("button"))
    save_btn.grid(row=0, column=2, padx=5, pady=5, sticky="e")

    self.summary_text = ctk.CTkTextbox(self.summary_tab, wrap="word", font=get_font("textbox"))
